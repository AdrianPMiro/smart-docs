...
export default function App() {
  ...
  const handleDrop = useCallback((e, t) => {
    ...
    if (!allowed.includes(file.type)) {
      alert(
        `Formato no válido para ${t === "data" ? "DATA" : "PLANTILLA"}. ` +
        `Debe ser .docx, .xls o .xlsx`
      );
      setIsDraggingData(false); setIsDraggingTpl(false);
      return;
    }
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    console.log(
      "[React] → handleSubmit disparado, dataFile:", dataFile?.name,
      "tplFile:", tplFile?.name
    );
    ...
    try {
      console.log(
        "[React] → A punto de hacer fetch a http://127.0.0.1:8000/api/process"
      );
        ...
        <div className="file-upload-group">
          ...
          >
            ...
            <div className="file-upload-content">
              ...
              <span className="file-upload-badge">.docx .xls .xlsx</span>
              <span className="file-upload-action">
